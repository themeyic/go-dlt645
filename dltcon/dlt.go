...
func NewClient(p dlt645.ClientProvider, opts ...Option) *Client {
	ctx, cancel := context.WithCancel(context.Background())
	c := &Client{
		Client:         dlt645.NewClient(p),
...
func (sf *Client) AddGatherJob(r Request) error {
	...
	if r.SlaveID < dlt645.AddressMin || r.SlaveID > dlt645.AddressMax {
		return fmt.Errorf("dltcon: slaveID '%v' must be between '%v' and '%v'",
			r.SlaveID, dlt645.AddressMin, dlt645.AddressMax)
	}

	switch r.FuncCode {
	case dlt645.FuncCodeReadCoils, dlt645.FuncCodeReadDiscreteInputs:
		quantityMax = dlt645.ReadBitsQuantityMax
	case dlt645.FuncCodeReadInputRegisters, dlt645.FuncCodeReadHoldingRegisters:
		quantityMax = dlt645.ReadRegQuantityMax
	default:
...
func (sf *Client) procRequest(req *Request) {
	...
	case dlt645.FuncCodeReadDiscreteInputs:
	...
	// 16-bit access read
	case dlt645.FuncCodeReadHoldingRegisters:
	...
	case dlt645.FuncCodeReadInputRegisters:
		...
		// FIFO read
		//case dltcon.FuncCodeReadFIFOQueue:
